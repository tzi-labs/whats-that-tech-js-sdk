...
const _commonjsHelpers = require('./whats-that-tech-js-sdk.BJu3ubxk.cjs');
...
const __filename$1 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('shared/whats-that-tech-js-sdk.D9oEUfJx.cjs', document.baseURI).href)));
...
async function loadFingerprints(debug = false, customDir, customFile) {
  const packagedDistCorePath = path.resolve(__dirname$1, "core.json");
  const devDistCorePath = path.resolve(__dirname$1, "../../dist/core.json");
  if (debug) {
  ...
  if (customFile) {
    ...
    try {
      ...
      if (isUrl(customFile)) {
        ...
        const resolvedPath = path.resolve(process.cwd(), customFile);
  ...
  if (corePathToTry) {
    ...
    try {
      const content = await fsPromises__default.readFile(corePathToTry, "utf-8");
...
async function findTech(options) {
  const { url, headless = true, timeout = 3e4, categories, excludeCategories, customFingerprintsDir, customFingerprintsFile, debug = false, onProgress } = options;
  onProgress?.({
    current: 1,
    total: 1,
    currentUrl: url,
...
async function detectTechnology(page, fingerprint, pageLoadCookies = []) {
...
if ((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('shared/whats-that-tech-js-sdk.D9oEUfJx.cjs', document.baseURI).href)) === `file://${process.argv[1]}`) {
