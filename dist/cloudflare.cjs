...
async function importDebug() {
    if (!debugModule) {
        debugModule = (await import('./chunks/index.cjs').then(function (n) { return n.index; })).default;
    }
    return debugModule;
}
/**
...
async function fetchCustomFingerprints(url) {
  if (!isUrl(url)) {
  ...
  try {
    ...
    console.log(`Successfully loaded ${Object.keys(data).length} custom fingerprints from ${url}`);
    return data;
  } catch (error) {
...
async function findTech(options, env) {
  ...
  if (customFingerprintsFile) {
    ...
    if (customFingerprints) {
      activeFingerprints = customFingerprints;
    }
  }
